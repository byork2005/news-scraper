...
$(document).on('click', '.notes-btn', function() {
  ...
  const notePanel = $(`<div class="panel panel-default">`);
  const notePanelBody = $(`<div class="panel-body">`);
  const deleteBtn = $(
    `<button type="submit" class="btn btn-danger note-delete" data-id="${id}">Delete</button>`
  );
  const saveBtn = $(
    `<button type="submit" class="btn btn-success note-save" data-id="${id}">Save Note</button>`
  );
  $('#note-btn-div').empty();
  $('#note-body-div').empty();
  $('#note-btn-div').append(saveBtn);
  $.ajax({
    method: 'GET',
    url: '/saved/' + id
  }).done(function(data) {
    console.log('data from saved notes: ', data);
  });
  $('#myModal').modal();
});
